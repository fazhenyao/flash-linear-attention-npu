...
{   
    ...
    // Scratch stays at the front; the recurrence state is resident at the UB tail.
    // This prevents gatedQ/dv2 scratch reuse from clobbering the cross-chunk state.
    uint64_t offsetDh = 0;
    const uint64_t residentOffset = this->totalTbufByte - this->dhBufSize * FLOAT_DTYPE_SIZE;
    this->bdhCastLocal = this->vecTbuf.template GetWithOffset<float>(this->dhBufSize, residentOffset);
...
__aicore__ inline void GDRVec<DT, GT>::InitLastDh()
{
    ...
    if (this->hasDht) {
        ...
        Duplicate(this->bdhCastLocal, 0.0f, this->dhBufSize);
...
__aicore__ inline void GDRVec<DT, GT>::CalcGatedQ(float& gLast, float& gLastExp, const bool isLastChunk)
{
    ...
    if (!this->hasG) {
        ...
        CopyInRaw(this->qLocal, this->qGm[gmOffsetK_ + bos_ * this->K], this->curCalcTK);
        CopyOutRaw(this->gatedQGm[gatedQOffset_], this->qLocal, this->curCalcTK);
        CrossCoreSetFlag<0x2, PIPE_MTE3>(CROSS_CORE_V2C_GQ);
        return;
    }
                                                ...
                                                const bool isLastChunk)
{
    ...
    {
        CopyIn(this->qdoCastLocal, this->qdoCastLocal, this->wv2Gm[wV2Offset_], this->dhBufSize, false);
        Sub(this->bdhCastLocal, this->bdhCastLocal, this->qdoCastLocal, this->dhBufSize);
    }
    if (chunkIdx_ == 0) {
