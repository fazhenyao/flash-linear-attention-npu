...
public:
    ...
    ge::graphStatus CheckInputDtype()
    {
        ...
        if (ctx_.hasG && gDtype != qDtype && gDtype != ge::DT_FLOAT) {
            OP_LOGE(ctx_.nodeName, "gDtype must be DT_FLOAT or as same as qDtype");
            return ge::GRAPH_FAILED;
        }
        if (ctx_.hasGk && ctx_.gkDtype != qDtype && ctx_.gkDtype != ge::DT_FLOAT) {
            OP_LOGE(ctx_.nodeName, "gk must be DT_FLOAT or have the same dtype as q.");
            return ge::GRAPH_FAILED;
        }
        if (ctx_.hasG && ctx_.hasGk && ctx_.gkDtype != gDtype) {
            OP_LOGE(ctx_.nodeName, "gk must have the same dtype as g when g is provided.");
            return ge::GRAPH_FAILED;
        }
        if ((ctx_.hasG && gDtype == ge::DT_FLOAT) || (!ctx_.hasG && ctx_.hasGk && ctx_.gkDtype == ge::DT_FLOAT)) {
    ...
    ge::graphStatus CalcUb()
    {
        ...
        const uint32_t dhScratchPeak = dhCastBufByte +
                                       halfK * static_cast<uint32_t>(V_) * CHUNK_GDR_BWD_DHU_HALF_DTYPE_SIZE;
        const uint32_t scratchPeak = std::max(dhScratchPeak, std::max(dvPeak, gatedQPeak));
