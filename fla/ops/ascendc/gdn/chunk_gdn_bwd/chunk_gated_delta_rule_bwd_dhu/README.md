...
| `gOptional` | 输入 | 可选 | 标量 Gate 张量 | 提供时对隐藏状态递推施加 base-2 门控 `exp2(g)`；省略时门控因子为 1 | `FLOAT16`、`BFLOAT16`、`FLOAT` | `ND` | `[B, HV, T]` | 支持 |
| `gkOptional` | 输入 | 可选 | Key-wise Gate 张量 | 对每个 Key 维度施加 base-2 门控 `exp2(gk)` | `FLOAT16`、`BFLOAT16`、`FLOAT`（`g` 存在时与 `g` 同 dtype） | `ND` | `[B, HV, T, K]` | 支持 |
...
- `h0Optional`、`dhtOptional` 的形状必须为 `[N, HV, K, V]`（若提供）；定长模式 `N=B`，变长模式 `N=len(cuSeqlensOptional)-1`。
...
- **TilingKey**：有效 gate dtype（优先 `g`，否则 `gk`）与 `q` 同 dtype 时为 Key=1，FP32 时为 Key=2；`g`/`gk` 均省略时为 Key=1（与 V 维无关）。
...
- `gOptional`：可省略；若提供，数据类型可以为 `FLOAT16`、`BFLOAT16` 或 `FLOAT`，且需与 `q` 类型一致或为 `FLOAT`（FP32）。省略时 `dv2` 和 `dh` 的标量 gate 因子均按 1 处理。
- `gkOptional`：若提供，形状为 `[B, HV, T, K]`，数据类型需与 `q` 一致或为 FP32；当 `gOptional` 同时存在时二者 dtype 必须一致。其最后一个 token 的 gate 以 `exp2` 作用于对应 K 行。
...
- A2/A3 性能路径将 FP32 反向递推状态放在独立的 UB resident 区，生命周期覆盖同一 head 的逆序 chunk 扫描；gatedQ/dv2/term scratch 仍复用 UB 前部。这样避免每个 chunk 把状态写回再读入 GM workspace，同时保持 resident 与 scratch 生命周期隔离。
- Cube 侧 term1/term2 使用独立 L1/L0 operand，并以硬事件替代不必要的全流水屏障，使 term1 写回与 term2 operand 搬运尽量重叠。
