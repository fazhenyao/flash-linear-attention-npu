...
static aclnnStatus CheckShape(ChunkFwdOParams params)
{
    ...
    // 维度数：q/k/v 4D、h 5D、g 3D、oOut 4D（README §3.1）
    ...
    // h 对齐：B/HV 与 v 一致；末两维恒为 [K, V]（v26.9.0 接口无 state_v_first）；
    // numChunks 维不校验（varlen 下由 chunkOffsets 决定）
    const int64_t hK = hShape.GetDim(3);
    const int64_t hV = hShape.GetDim(4);
    CHECK_COND(hShape.GetDim(0) == vShape.GetDim(0) && hShape.GetDim(1) == vShape.GetDim(1) &&
               ...
               ACLNN_ERR_PARAM_INVALID, "h should be [B, HV, numChunks, K, V] aligned with q and v.");

    // oOut 恒为 [B, HV, T, V]（v26.9.0 接口无 output_layout；tiling 层无 oOut 信息）
    CHECK_COND(oShape.GetDimNum() == CHUNK_FWD_O_QKV_DIM_NUM, ACLNN_ERR_PARAM_INVALID,
               "oOut should be 4D [B, HV, T, V].");
    CHECK_COND(oShape.GetDim(0) == vShape.GetDim(0) && oShape.GetDim(1) == vShape.GetDim(1) &&
               ...
               ACLNN_ERR_PARAM_INVALID, "oOut should be [B, HV, T, V] same as v.");
