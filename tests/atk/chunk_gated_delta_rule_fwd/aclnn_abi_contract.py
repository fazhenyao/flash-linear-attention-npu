...
def main() -> None:
    ...
    for forbidden in ("ChunkGatedDeltaRuleFwdPrepare", "l0op::ChunkFwdH", "l0op::ChunkFwdO"):
        if forbidden in implementation:
            raise SystemExit(f"Unexpected external operator dependency: {forbidden}")
    if "!params.useExp2 && !params.useQkL2norm" not in implementation:
        raise SystemExit("Missing unsupported feature guard")
