...
void CheckInputs(const at::Tensor &q, const at::Tensor &k, const at::Tensor &w, const at::Tensor &d_o,
                 const at::Tensor &dv, const c10::optional<at::Tensor> &g, const c10::optional<at::Tensor> &gK,
                 const c10::optional<at::Tensor> &h0, const c10::optional<at::Tensor> &dht, at::OptionalIntArrayRef cu_seqlens,
                 at::OptionalIntArrayRef chunk_indices)
{
    ...
    if (g.has_value()) {
        CheckInputRank(g.value(), 3, "g");
        CheckSameDevice(q, g.value(), "g");
    }
    ...
    if (g.has_value()) {
        TORCH_CHECK(g.value().size(0) == B && g.value().size(1) == Hv && g.value().size(2) == T,
                  "g must be [B,Hv,T]; g=", g.value().sizes());
    }
    int64_t stateBatch = cu_seqlens.has_value() ? static_cast<int64_t>(cu_seqlens.value().size()) - 1 : B;
    if (gK.has_value()) {
        ...
        TORCH_CHECK(gK.value().scalar_type() == q.scalar_type() || gK.value().scalar_type() == at::kFloat,
                    "gK must have the same dtype as q or be float32.");
        TORCH_CHECK(!g.has_value() || gK.value().scalar_type() == g.value().scalar_type(),
    ...
    if (g.has_value()) {
        auto gDtype = g.value().scalar_type();
...
ChunkGatedDeltaRuleBwdDhuTilingResult CalcTilingParams(
    const at::Tensor &q, const at::Tensor &k, const at::Tensor &w, const at::Tensor &d_o, const at::Tensor &dv,
    const c10::optional<at::Tensor> &g, const c10::optional<at::Tensor> &gK, const c10::optional<at::Tensor> &h0,
    ...
    at::OptionalIntArrayRef chunk_indices)
{
    ...
    gert::StorageShape *gShapePtr = nullptr;
    ...
    if (g.has_value()) {
        auto sizes = g.value().sizes();
        gShape = gert::StorageShape({sizes[0], sizes[1], sizes[2]}, {sizes[0], sizes[1], sizes[2]});
        gShapePtr = &gShape;
    }
    if (gK.has_value()) {
    ...
    optiling::ChunkGatedDeltaRuleBwdDhuTilingContext ctx{
        ...
        g.has_value() ? ToGeDtype(g.value().scalar_type()) : ge::DT_FLOAT,
    ...
    at::OptionalIntArrayRef chunk_indices)
{
    ...
    CheckInputs(q, k, w, d_o, dv, g, gK, h0, dht, cu_seqlens, chunk_indices);
    ...
    auto tilingResult = CalcTilingParams(q, k, w, d_o, dv, g, gK, h0, dht, scale, chunk_size,
                                         cu_seqlens, chunk_indices);
    ...
    GM_ADDR gPtr = g.has_value() ? (GM_ADDR)g.value().data_ptr() : nullptr;
    ...
    auto qDtype = q.scalar_type();
    auto gateDtype = g.has_value() ? g.value().scalar_type() :
                     (gK.has_value() ? gK.value().scalar_type() : q.scalar_type());
    auto tiling = tilingResult.tiling;
    auto blockDim = tilingResult.blockDim;

    auto aclCall = [=]() -> int {
        if (qDtype == at::kBFloat16 && gateDtype == at::kBFloat16) {
        ...
        } else if (qDtype == at::kHalf && gateDtype == at::kHalf) {
            LaunchChunkGatedDeltaRuleBwdDhu<half, half>(
                blockDim, stream, qPtr, kPtr, wPtr, doPtr, dvPtr, gPtr, gkPtr, h0Ptr, dhtPtr, cuSeqlensPtr, chunkIndicesPtr, dhPtr, dh0Ptr,
                dv2Ptr, workspaceGm, tiling);
        } else if (qDtype == at::kBFloat16 && gateDtype == at::kFloat) {
            LaunchChunkGatedDeltaRuleBwdDhu<bfloat16_t, float>(
                blockDim, stream, qPtr, kPtr, wPtr, doPtr, dvPtr, gPtr, gkPtr, h0Ptr, dhtPtr, cuSeqlensPtr, chunkIndicesPtr, dhPtr, dh0Ptr,
                dv2Ptr, workspaceGm, tiling);
        } else if (qDtype == at::kHalf && gateDtype == at::kFloat) {
            LaunchChunkGatedDeltaRuleBwdDhu<half, float>(
                blockDim, stream, qPtr, kPtr, wPtr, doPtr, dvPtr, gPtr, gkPtr, h0Ptr, dhtPtr, cuSeqlensPtr, chunkIndicesPtr, dhPtr, dh0Ptr,
                dv2Ptr, workspaceGm, tiling);
        } else {
            TORCH_CHECK(false, "Unsupported dtype combination: q=", qDtype, ", gate=", gateDtype);
        }
        return 0;
    };
